  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-3">
      ...
      <div className="inline-flex rounded-lg overflow-hidden border" style={{ borderColor: '#E8DED1' }}>
        {(['category', 'store'] as const).map((mode) => (
          <button key={mode}
            ...
            style={{
              background: filters.viewMode === mode ? '#4A3728' : '#fff',
              color: filters.viewMode === mode ? '#fff' : '#6B5B4F',
            }}>
      ...
      <div className="inline-flex rounded-lg overflow-hidden border" style={{ borderColor: '#E8DED1' }}>
        {(['monthly', 'quarterly', 'annual'] as const).map((g) => (
          <button key={g}
            ...
            className="px-3 py-1.5 text-[10px] font-medium uppercase tracking-[0.15em] transition-colors"
            style={{
              background: filters.granularity === g ? '#4A3728' : '#fff',
              color: filters.granularity === g ? '#fff' : '#6B5B4F',
            }}>
      ...
      <div className="flex gap-1.5">
        {YEARS.map((y) => (
          <button key={y}
            ...
            style={{
              background: filters.year === y ? '#4A3728' : '#fff',
              color: filters.year === y ? '#fff' : '#6B5B4F',
              border: `1px solid ${filters.year === y ? '#4A3728' : '#E8DED1'}`,
            }}>
            {y}
          </button>
        ))}
      ...
      <div className="w-full flex flex-wrap gap-1 sm:hidden">
        {MONTHS.map((m) => (
          <button key={m.label}
            ...
            style={{
              background: filters.month === m.value ? '#4A3728' : 'transparent',
              color: filters.month === m.value ? '#fff' : '#6B5B4F',
            }}>
            {m.label}
          </button>
        ))}
      ...
      <select
        ...
        style={{ borderColor: '#E8DED1', color: '#282627', background: '#fff' }}>
        {MONTHS.map((m) => (
